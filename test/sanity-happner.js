describe('sanity-happner', function() {
  var should = require('chai').should();

  var service = require('happner-2');
  ...
  var config = require('./fixtures/happner-config').get(db_path, test_id);
    ...
    try {
      service.create(
        ...
        function(e, mesh) {
          ...
          dataComponent = meshInstance.exchange.data;

          dataEvents = meshInstance.event.data;
  ...
  context('direct use', function() {
    ...
    it('should subscribe and get an initial value on the callback', function(callback) {
      dataComponent.set('/b7/testsubscribe/data/value_on_callback_test', { test: 'data' }, function(
        e
      ) {
        if (e) return callback(e);
    ...
    it('should subscribe and get initial values emitted immediately', function(callback) {
      ...
      dataComponent.set('/b7/testsubscribe/data/values_emitted_test/1', { test: 'data' }, function(
        e
      ) {
        if (e) return callback(e);
